...
export class Algorithm {
  ...
  efficiency(p, T, do_loop) {
    ...
    while (orbit.indexOf(tc) == -1) {
      ...
      tc = this.argmin(feasible, function (el, idx) {
        return el + p
      })[0]
      if (do_loop) {
        tc = tc % tl
  ...
  profile(V, r, T, do_loop) {
    ...
    if (this.inner(r, r) == 0) {
      V = V.map((v) => [
        v,
        ((eff = this.efficiency(v[1], T, do_loop)),
        (this.inner(v[2], v[2]) * eff[0]) / eff[1]),
        eff[0],
        eff[1],
      ])
    } else {
  ...
  optimize2(config) {
    ...
    let Vf = this.V.filter(function (v) {
      ...
      return (
        config.get_min_time() <= v[1] &&
        v[1] <= config.get_max_time() &&
        config.get_min_level() <= lv &&
        lv <= config.get_max_level()
      )
    })
  ...
  _optimize2(Vf, r, is_zero_rate, path, best_group) {
    ...
    if (path.length >= 4) {
      ...
      for (let i = 0; i < r.length; ++i) {
        vsum[i] = 0.0
        for (let n = 0; n < 4; ++n) vsum[i] += Vf[path[n]][1][i]
      }
      ...
      let sidx
      if (path.length == 0) sidx = 0
      else sidx = path[path.length - 1] + 1
  ...
  __score(v, r, is_zero_rate) {
    if (is_zero_rate) {
      ...
      if (this.inner(u, u) == 0) return 0
      // 두 벡터의 내적 + 코사인 유사도 * lambda를 반환한다.
      else {
        let score_len = this.inner(u, s)
        let score_cos = this.inner(u, s, bnd) / Math.sqrt(this.inner(u, u, bnd))
...
export class ResultView {
  ...
  createTable() {
    ...
    for (let i = 0; i < 7; ++i)
      row.appendChild(document.createElement('th')).style.width = '60px'
  ...
  updateTable(result_groups, table_id) {
    ...
    for (let n = 0; n < 4; ++n) {
      ...
      if (group) {
        ...
        // 작전주기
        tr.cells[1].childNodes[0].childNodes[0].innerHTML =
          Util.integer_to_hhmm(operation[1])

        // 작전 빈도 (분자)
        tr.cells[1].childNodes[1].childNodes[0].innerHTML = group[2] + ''

        // 작전 빈도 (분모)
        tr.cells[1].childNodes[1].childNodes[3].innerHTML = group[3] + ''
        ...
        for (let i = 0; i < 5; ++i) {
          ...
          if (resource[4 + i] > 0) {
            ...
            else tr.cells[6].innerHTML += '<font size="1"> or </font>'
        ...
        if (!first)
          tr.cells[6].innerHTML +=
            "<span class='smalltext'>x" + group[2] + '</span>'
      } else {
        // When there is no result
        for (let i = 0; i < 7; ++i) tr.cells[i].innerHTML = '-'
      }
    }
  ...
  createSpecialDropImage(category) {
    ...
    switch (category) {
      case 0:
        out += './img/quick_reinforce.png'
        break
      case 1:
        out += './img/quick_develop.png'
        break
      case 2:
        out += './img/iop_contract.png'
        break
      case 3:
        out += './img/equip_contract.png'
        break
      case 4:
        out += './img/furniture_coin.png'
        break
      default:
        throw 'illegal special drop category: ' + category
    }
    out += '" />'
    return out
  }
