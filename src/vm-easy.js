...
class VMEasy {
	constructor() {
		...
		//  쾌속수복, 쾌속제조, 인형, 장비, 토큰
		//  기상시각, 취침시각, 0지개방]
		this.inputs = document.getElementById('easy_config')
		...
		let btfn = (evt) => {
			if(evt.target.checked)
				this.__set_button_state(evt.target, 0);
			else
				this.__set_button_state(evt.target, 1);
		};
		for(let i = 0; i < 9; ++i) {
	...
	update(cfg) {
		...
		if(cfg.timeline.length == 0) {
			...
			for(let i = 0; i < cfg.timeline.length; ++i) {
				let newitv = cfg.timeline[(i + 1) % cfg.timeline.length] - cfg.timeline[i] + 1440 * Math.floor((i+1)/cfg.timeline.length);
				if(maxitv < newitv) {
					maxidx = i;
					maxitv = newitv;
				}
	...
	fetch() {
		...
		for(let i = 4; i < 9; ++i)
			if(this.inputs[i].checked)
				precfg.ratio[i] = Algorithm.CONTRACTION_MID;
