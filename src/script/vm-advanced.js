...
export class VMAdvanced {
  ...
  constructor() {
    ...
    for (let i = 0; i < 4; ++i) this.ratio[i].onchange = this.auto_fix_float
    ...
    document.getElementById('bt-repeat-add').onclick = (evt) => {
      ...
      let stime = Util.ask_via_prompt(
        Util.is_valid_hhmm,
        LanguageManager.instance.get_word(17),
        LanguageManager.instance.get_word(18),
        '00:00'
      )
      if (stime == null) return
      stime = Util.hhmm_to_integer(stime)

      let itime = Util.ask_via_prompt(
        Util.is_valid_hhmm,
        LanguageManager.instance.get_word(19),
        LanguageManager.instance.get_word(20),
        '1:00'
      )
      if (itime == null) return
      itime = Util.hhmm_to_integer(itime)

      let etime = Util.ask_via_prompt(
        (tstr) => {
          return Util.is_valid_hhmm(tstr) && Util.hhmm_to_integer(tstr) >= stime
        },
      ...
      while (stime <= etime && stime < 1440) {
        this.__tb_addRow(-1).cells[0].childNodes[0].value =
          Util.integer_to_hhmm(stime)
        stime += itime
      }
    }
  }
  ...
  update(cfg) {
    // config table
    for (let idx = 0; idx < 4; ++idx) this.ratio[idx].value = cfg.ratio[idx]
  ...
  fetch() {
    ...
    for (let r = 0; r < this.timeline.rows.length; ++r) {
      precfg.timeline[r] = Util.hhmm_to_integer(
        this.__tb_dom(r, 0).childNodes[0].value
      )
    }
  ...
  __tb_dom(r, c) {
    console.assert(r >= 0)
    if (c === undefined) return this.timeline.rows[r]
    else {
      console.assert(c >= 0)
  ...
  __tb_index_of(tr_dom) {
    ...
    for (let i = 0; i < this.timeline.rows.length; ++i)
      if (this.__tb_dom(i) === tr_dom) return i
    return null
  }
  ...
  __intensity_to_float(s) {
    ...
    switch (parseInt(s)) {
      case 0:
        return Algorithm.CONTRACTION_IGNORE
      case 1:
        return Algorithm.CONTRACTION_LOW
      case 2:
        return Algorithm.CONTRACTION_MID
      case 3:
        return Algorithm.CONTRACTION_HIGH
      default:
        throw 'illegal state'
    }
  }
  ...
  __float_to_intensity(x) {
    console.assert(x >= 0)
    if (x <= Algorithm.CONTRACTION_IGNORE) return 0
    else if (x <= Algorithm.CONTRACTION_LOW) return 1
    else if (x <= Algorithm.CONTRACTION_MID) return 2
    else if (x <= Algorithm.CONTRACTION_HIGH) return 3
    else throw 'illegal parameter ' + x
  }
}
