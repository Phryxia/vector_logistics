...
export class PresetController {
    ...
    this.view = new PresetView(
      ...
      (evt) => {
        ...
        let preset_name = window.prompt(
          LanguageManager.instance.get_word(33),
          'nice-name'
        )
        ...
        while (preset_name !== null && !pass) {
          if (preset_name === '')
            preset_name = window.prompt(
              LanguageManager.instance.get_word(34),
              'nice-name'
            )
          else if (preset_name.includes(';'))
            preset_name = window.prompt(
              LanguageManager.instance.get_word(35),
              'nice-name'
            )
          else pass = true
        }
        ...
        if (preset_name !== null && preset_name != '') {
          ...
          this.add_preset(
            new Preset(preset_name, this.cfgctr.fetch(), this.cfgctr.mode)
          )
        }
      },
  ...
  override_presets(presets, selected_index) {
    ...
    for (let i = 0; i < presets.length; ++i)
      if (!presets[i] || !(presets[i] instanceof Preset))
        throw (
          '[PresetController::override_presets] illegal preset: ' + presets[i]
        )
    ...
    if (selected_index === undefined || selected_index < 0) selected_index = 0
  ...
  /**
	...
	*/
  get_current() {
    if (this.selected_index >= 0) return this.presets[this.selected_index]
    else return null
  }
  ...
  del_preset(idx) {
    ...
    this.presets = this.presets
      .slice(0, idx)
      .concat(this.presets.slice(idx + 1))
    this.view.update(this.presets, this.selected_index)
    CookieManager.instance.save_snapshot(this)
  }
}
