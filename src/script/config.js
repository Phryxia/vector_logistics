...
export class Config {
  ...
  get_ratio(do_normalize) {
    let denom = 1
    if (do_normalize) {
      denom = this.ratio[0] + this.ratio[1] + this.ratio[2] + this.ratio[3]
...
export class ConfigController {
  constructor() {
    ...
    this.div = [
      document.getElementById('easy_config'),
      document.getElementById('advanced_config'),
    ]

    // 간편모드 버튼, 고급 모드 버튼
    this.btn = [
      document.getElementById('mode-sel-0'),
  ...
  set_mode(mode, no_alert) {
    ...
    if (!no_alert && this.mode == 1 && mode == 0)
      if (!window.confirm(LanguageManager.instance.get_word(43))) return
